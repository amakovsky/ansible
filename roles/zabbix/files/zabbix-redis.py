...
def stats(location, type, password):
    ...
    rc, output = execute('/usr/local/bin/redis-cli %(opts)s INFO %(section)s' % {
        'opts': opts,
...
def stats_sentinel(opts, master_name, prefix):
    result = {}
    rc, output = execute('/usr/local/bin/redis-cli %(opts)s SENTINEL ckquorum %(name)s' % {
        'opts': opts,
        'name': master_name,
    })
    if rc == 0:
...
def stats_cluster(opts):
    result = {}
    rc, output = execute('/usr/local/bin/redis-cli %(opts)s CLUSTER INFO' % {
        'opts': opts,
    })
    if rc == 0:
        for line in output.splitlines():
